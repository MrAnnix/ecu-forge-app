...
class DebugUsbTelemetryTransportGatewayTest {
    @Test
    fun connectRejectsBluetoothEndpointWithInvalidEndpointError() {
        runBlocking {
            val gateway = DebugUsbTelemetryTransportGateway()

            val result = gateway.connect(TransportEndpoint.Bluetooth("AA:BB:CC:DD:EE:FF"))

            assertThat(result)
                .describedAs("Debug USB telemetry gateway should reject non-USB endpoints")
        ...
        runBlocking {
            val gateway =
                DebugUsbTelemetryTransportGateway(
                    behavior = DebugUsbTelemetryTransportGateway.Behavior.CONNECT_FAILURE,
                )
    ...
    fun readTimeoutBehaviorReturnsTimeoutErrorAfterSuccessfulCommandWrite() {
        runBlocking {
            ...
            gateway.write("TELEMETRY_SNAPSHOT?;FAMILY=KEIHIN;HINT=USB".encodeToByteArray())
