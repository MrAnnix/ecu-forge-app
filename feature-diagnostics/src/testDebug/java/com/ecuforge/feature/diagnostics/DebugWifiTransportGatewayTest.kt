...
class DebugWifiTransportGatewayTest {
    @Test
    fun connectRejectsUsbEndpointWithInvalidEndpointError() {
        runBlocking {
            val gateway = DebugWifiTransportGateway()

            val result = gateway.connect(TransportEndpoint.Usb(vendorId = 1027, productId = 48960))

            assertThat(result)
                .describedAs("Debug WiFi gateway should reject non-WiFi endpoints")
        ...
        runBlocking {
            val gateway =
                DebugWifiTransportGateway(
                    behavior = DebugWifiTransportGateway.Behavior.CONNECT_FAILURE,
                )
    ...
    fun readTimeoutBehaviorReturnsTimeoutErrorAfterSuccessfulCommandWrite() {
        runBlocking {
            ...
            gateway.write("ID?;FAMILY=KEIHIN;HINT=WIFI".encodeToByteArray())
