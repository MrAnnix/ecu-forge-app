...
/**
 * Debug-only concrete Bluetooth gateway used to pilot transport-backed diagnostics flows.
 *
 * @property behavior Deterministic behavior profile for nominal and failure-path testing.
 */
internal class DebugBluetoothTransportGateway(
    ...
    override suspend fun connect(endpoint: TransportEndpoint): TransportOperationResult<Unit> {
        if (endpoint !is TransportEndpoint.Bluetooth) {
            ...
            return TransportOperationResult.failure(
                ...
                message = "Debug Bluetooth gateway requires a Bluetooth endpoint",
                recoverable = false,
            )
        }
        ...
        if (behavior == Behavior.CONNECT_FAILURE) {
            ...
            return TransportOperationResult.failure(
                ...
                message = "Debug Bluetooth connection failed",
            )
        }
    ...
    override suspend fun write(payload: ByteArray): TransportOperationResult<Int> {
        if (state != TransportConnectionState.CONNECTED) {
            return TransportOperationResult.failure(
                ...
                message = "Debug Bluetooth gateway is not connected",
            )
        }

        val command = payload.toString(Charsets.UTF_8)
        lastCommand = command
    ...
    override suspend fun read(maxBytes: Int): TransportOperationResult<ByteArray> {
        ...
        if (behavior == Behavior.READ_TIMEOUT) {
            return TransportOperationResult.failure(
                ...
                message = "Debug Bluetooth read timeout",
            )
        }
        ...
        val payload =
            when {
                command.startsWith("ID?") -> "MODEL=KM602EU|FW=2.10.4|SN=A1B2C3".encodeToByteArray()
                command.startsWith("DTC?") -> "P0130;O2 Sensor Circuit|P0301;Cylinder 1 Misfire".encodeToByteArray()
                    ...
                    return TransportOperationResult.failure(
                        ...
                        message = "Debug Bluetooth gateway cannot respond to command: $command",
                        recoverable = false,
                    )
                }
            }
